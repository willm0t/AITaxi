...
class Taxi:
    ...
    def get_traffic(self, node_coords):
        node = self._world.getNode(node_coords[0], node_coords[1])
        if node:
            return node.traffic
        else:
            return None
    ...
    def _heuristic(self, node, destination):
        ...
        # Use the maximum of the horizontal or vertical difference to calculate the diagonal distance,
        # then add the straight-line distance for the remainder.
        # A 'tie-breaking' heuristic, mix between Manhattean and Euclidean
        return D * (dx + dy) + (D2 - 2 * D) * min(dx, dy)

    def _planPath(self, start, target):
        # basic checks for error handling
        ...
        # continues  as long as node is in priority queue
        while priority_queue:
            cost, current_node, path = heapq.heappop(priority_queue)
            # skip current node if already explored
            ...
            for next_node, (cost_value, _) in self._map[current_node].items():
                # consider moving to neighbour if not explored
                if next_node not in explored:
                    # get dynamic traffic penalty for next node
                    traffic_penalty = self.get_traffic(next_node)

                    # Calculate the estimated cost
                    estimated_cost = cost + cost_value + self._heuristic(next_node, target) + traffic_penalty
                    heapq.heappush(priority_queue, (estimated_cost, next_node, path + (next_node,)))
    ...
    # currently only deterministic, need probabilistic
    def _bidOnFare(self, time, origin, destination, price):
        # calc time to origin and destination
        TimeToOrigin = self._world.travelTime(self._loc, self._world.getNode(origin[0], origin[1]))
        TimeToDestination = self._world.travelTime(self._world.getNode(origin[0], origin[1]), self._world.getNode(destination[1], destination[1]))

        #check current passengers and allocated fares
        ...
        # confirm travel times are not 0
        FiniteTimeToOrigin = TimeToOrigin > 0
        FiniteTimeToDestination = TimeToDestination > 0

        # max time to make taxis less likely to take fares from distances further than 25 ticks.
        max_time = 25
        BiddingProbability = max(0,1 - (TimeToOrigin / max_time))
        randomNum = random.random()

        # make a probabilistic decision to bid
        ProbabilisticDecision = randomNum < BiddingProbability

        # assess financial and timing feasibility of taking to fare
        CanAffordToDrive = self._account > TimeToOrigin # Include distance in cost calculation
        FairPriceToDestination = price > TimeToDestination  # Include distance in revenue calculation
        PriceBetterThanCost = FairPriceToDestination and FiniteTimeToDestination > 0
        ...
        # Bid decision
        Bid = ProbabilisticDecision and NotCurrentlyBooked and CanAffordToDrive and WillArriveOnTime and PriceBetterThanCost
        return Bid
